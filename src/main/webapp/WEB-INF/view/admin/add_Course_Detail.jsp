            ...
            <div class="tab">
                ...
                <div style="width: 100%; display: flex">
                    <div style="width: 70%;vertical-align: text-bottom;">
                        ...
                        <hr>
                        <b><label style="font-size: 18px">วัตถุประสงค์</label></b>
                        <span id="displayObjectives"></span>
                        <hr>
                        ค่าธรรมเนียม : <span id="displayFee" style="margin-right: 20px"></span>
                        ระยะเวลาในการเรียน : <span id="displayTotalHours" style="margin-right: 20px"></span> ชั่วโมง
                        ไฟล์หลักสูตร : <span id="displayCourseFile"></span>
                    </div>
                    <div style="width: 30%; writing-mode: vertical-rl;">
                        <img id="displayPreview" src="" alt="Image Preview" style="display: none; height: 500px; margin-top: 10px; border-radius: 10px">
                    </div>
                </div>
            </div>
...
<script>
    function checkScriptPage1(frm){
        //------------course_type--------------
        if (frm.course_type.value === "") {
        ...
        if(frm.course_name.value === ""){
            ...
            return false;
        }else if (!frm.course_name.value.match(regCName)) {
        ...
        if (frm.certificateName.value === ""){
            ...
            return false;
        } else if (!frm.certificateName.value.match(regCerName)) {
            // alert("ตัองเป็นภาษาไทย อังกฤษหรือตัวเลขเท่านั้น");
        ...
        checkScriptPage1IMG(frm);

    }
    function checkScriptPage1IMG(frm){
        if (frm.course_img.files.length === 0) {
    ...
    function checkScriptPage2(frm){
        ...
        //---------CourseTotalHours----------
        if (frm.course_totalHours.value === ""){
            // alert("กรุณากรอกชื่อเกียรติบัตร");
            ...
            return false;
        } else if(parseInt(frm.course_totalHours.value) < 1 || parseInt(frm.course_totalHours.value) > 999999){
            // alert("ตัองเป็นภาษาไทย อังกฤษหรือตัวเลขเท่านั้น");
            document.getElementById("invalidCourseTotalHours").innerHTML = "ต้องมีค่าระหว่าง 1 - 999,999 บาท";
            document.getElementById("course_totalHours").focus();
            frm.course_totalHours.value = "";
            return false;
        }else {
    ...
    function checkScript(frm) {
        if (currentTab === 0) {
            // ตรวจสอบข้อมูลใน Step 1
            if (!checkScriptPage1(frm)) {
                return false;
            }
        }
        if (currentTab === 1) {
            // ตรวจสอบข้อมูลใน Step 2
            if (!checkScriptPage2(frm)) {
                return false;
            }
        }
        // เพิ่มเงื่อนไขการตรวจสอบข้อมูลในขั้นตอนอื่น ๆ ตามต้องการ
